...
import api.python.index
__now_variables = dir(api.python.index)

if 'handler' in __now_variables or 'Handler' in __now_variables:
    base = api.python.index.handler if ('handler' in __now_variables) else  api.python.index.Handler
    ...
    if (
        not inspect.iscoroutinefunction(api.python.index.app) and
        not inspect.iscoroutinefunction(api.python.index.app.__call__)
    ):
        ...
        def now_handler(event, context):
            ...
            response = Response.from_app(api.python.index.app, environ)
        ...
        def now_handler(event, context):
            ...
            response = asgi_cycle(api.python.index.app, body)
            return response

else:
    print('Missing variable `handler` or `app` in file api.python.index.py')
    print('See the docs https://zeit.co/docs/v2/deployments/official-builders/python-now-python')
    exit(1)
